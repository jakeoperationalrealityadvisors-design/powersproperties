...
function renderTeamGrid(settings){
  ...
  if(title) title.textContent = settings.teamTitle || "Meet Our Team";
  if(body) body.textContent = settings.teamBody || "The people behind the work.";
...
function applySettings(settings){
  ...
  setText("heroTitle", merged.heroTitle);
  setText("heroSubtitle", merged.heroSubtitle);
  applyGlobalContact(merged);
  setDisplay("aboutSection", merged.showAboutSection);
  setText("aboutTitle", merged.aboutTitle);
  setText("aboutBody", merged.aboutBody);
  setImageSrc("aboutImage", merged.aboutImage || DEFAULT_SETTINGS.aboutImage);
  const eventSection = document.getElementById("eventSection");
  if(eventSection) eventSection.style.display = merged.showEventSection ? "" : "none";
  const eventTitle = document.getElementById("eventTitle");
  const eventBody = document.getElementById("eventBody");
  if(eventTitle) eventTitle.textContent = merged.eventTitle || DEFAULT_SETTINGS.eventTitle;
  if(eventBody) eventBody.textContent = merged.eventBody || DEFAULT_SETTINGS.eventBody;
  renderGalleryList("eventGallery", merged.eventImages, merged.aboutImage || DEFAULT_SETTINGS.aboutImage);
  setDisplay("bottomSection", merged.showBottomSection);
  setText("bottomTitle", merged.bottomTitle);
  setText("bottomBody", merged.bottomBody);
  renderGalleryList("bottomGallery", merged.bottomImages, merged.aboutImage || DEFAULT_SETTINGS.aboutImage);
  renderTeamGrid(merged);
}
...
window.addEventListener("powersDataUpdated", function(event){
  if(event && event.detail){
    render(normalizeData(event.detail || DEFAULT_DATA));
    return;
  }
  loadSiteData();
});
...
function render(data){
  ...
  const featured=(data.rentals || []).filter(rental => rental.featured);
  setHtml('featured', featured.length ? featured.map((rental, index) => `
    <div class="card">
      ${renderCardCarousel(rental.images, rental.title, '', 'featured-' + (rental.id || index))}
      <div class="card-body">
        <h3>${escapeHtml(rental.title)}</h3>
        <p>${escapeHtml(rental.description || "")}</p>
        <div class="card-actions">
          <a href="property.html?id=${encodeURIComponent(rental.id)}" class="btn">View Details</a>
          <a href="mailto:${(data.settings && data.settings.email) || 'dustinpowers719@gmail.com'}" class="btn secondary">Inquire</a>
        </div>
      </div>
    </div>`).join("") : "<div class='empty-state'>No featured rentals yet.</div>");
  const posts=publishedContent(data, "home");
  setHtml('dynamicContent', posts.map(post => `
    <div class="social-post">
      <div class="social-post-head"><div class="composer-avatar">P</div><div><strong>Powers Properties</strong><p>Home Update</p></div></div>
