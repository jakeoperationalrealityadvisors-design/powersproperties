...
function applySettings(settings){
  ...
  document.getElementById("teamPageTitle").textContent = merged.teamTitle || DEFAULT_SETTINGS.teamTitle;
  document.getElementById("teamPageBody").textContent = merged.teamBody || DEFAULT_SETTINGS.teamBody;
  const callBtn = document.getElementById("teamPageCallBtn");
  if(callBtn){
    callBtn.href = "tel:" + digitsOnly(merged.primaryPhone || contactDefaults().primaryPhone);
  }
...
function render(data){
  ...
  const posts = publishedContent(data, "team");
  const dynamic = document.getElementById("dynamicContent");
  dynamic.innerHTML = posts.length ? posts.map(post => `
    <div class="social-post">
      <div class="social-post-head"><div class="composer-avatar">P</div><div><strong>Powers Properties</strong><p>Team Update</p></div></div>
