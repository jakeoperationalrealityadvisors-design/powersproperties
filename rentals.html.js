...
function renderSocialPosts(posts){
  return posts.map(post => `
    <div class="social-post">
      <div class="social-post-head"><div class="composer-avatar">P</div><div><strong>Powers Properties</strong><p>Rental Update</p></div></div>
...
function render(data){
  ...
  el.innerHTML=(data.rentals || []).length ? data.rentals.map((rental, index) => {
    const status=rental.available ? "Available" : "Rented";
    const badgeClass=rental.available ? "available" : "rented";
    return `
      <div class="card">
        ${renderCardCarousel(rental.images, rental.title, `<span class="badge ${badgeClass}">${status}</span>`, 'rental-' + (rental.id || index))}
        <div class="card-body">
          <h3>${escapeHtml(rental.title)}</h3>
          <p>${escapeHtml(rental.description || "")}</p>
          ${rental.price ? `<p style="margin-top:6px; font-weight:600; color:#fff;">${escapeHtml(rental.price)}</p>` : ""}
          ${renderAmenitiesList(rental.amenities)}
  ...
  }).join("") : '<div class="empty-state">No rentals available right now.</div>';
  document.getElementById('dynamicContent').innerHTML = renderSocialPosts(publishedContent(data, "rentals"));
}
